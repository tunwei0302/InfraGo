...
void main() {
  ...
  test(
    'a user cannot save two locations with the same name (case-insensitive)',
    () {
      expect(sql, contains('CREATE UNIQUE INDEX'));
      expect(sql, contains('uq_weather_saved_locations_user_label'));
      expect(
        sql,
        contains('ON weather_saved_locations(user_id, lower(label))'),
      );
    },
  );

  test(
    'row level security is enabled with an owner-only policy for every CRUD operation',
    () {
      expect(
        sql,
        contains(
  ...
  test(
    'update policy checks ownership on both the existing and the new row',
    () {
      final start = sql.indexOf(
        'CREATE POLICY weather_saved_locations_owner_update',
      );
      final end = sql.indexOf(';', start);
      final policySql = sql.substring(start, end);
