...
void main() {
  ...
  test('the insecure 4-arg version (client-supplied user/points) is dropped', () {
    expect(sql, contains('DROP FUNCTION IF EXISTS earn_completion_reward(UUID, UUID, UUID, INTEGER)'));
  });

  test('the replacement only accepts ride_id and payment_id — never a user or points', () {
    final start = sql.indexOf('CREATE OR REPLACE FUNCTION earn_completion_reward');
  ...
  test('only a participant of the ride (rider or driver) may trigger it', () {
    expect(sql, contains('auth.uid() NOT IN (v_ride.rider_id, v_ride.driver_id)'));
    expect(sql, contains('not_a_ride_participant'));
  });

  test('points are computed from the payment\'s own final_amount, not trusted from the caller', () {
    expect(
      sql,
      contains('v_points := floor(COALESCE(v_payment.final_amount, 0) * 10)::INTEGER'),
    );
  });
  ...
  test('points are always credited to the ride\'s rider, never an arbitrary caller-supplied id', () {
    final insertIndex = sql.indexOf("VALUES (\n    v_ride.rider_id, p_ride_id, p_payment_id, 'earn'");
  ...
  test('function stays restricted to authenticated callers only', () {
    expect(sql, contains('REVOKE ALL ON FUNCTION earn_completion_reward(UUID, UUID) FROM PUBLIC'));
    expect(sql, contains('GRANT EXECUTE ON FUNCTION earn_completion_reward(UUID, UUID) TO authenticated'));
  });
}
