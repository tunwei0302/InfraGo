...
Map<String, dynamic> _fullRpcResult() => {
  ...
  'payments': [
    ...
    {'method': 'demo_wallet', 'status': 'paid', 'count': 4, 'total_myr': 41.0},
  ],
};
...
void main() {
  group('fetchSnapshot', () {
    test('parses a full RPC result into a snapshot', () async {
      ...
      expect(
        snapshot.serviceCategoryDistribution.first.serviceType,
        'economy_4',
      );
    ...
    test('throws SdgAnalyticsException when the RPC reports failure', () async {
      final repo = SdgAnalyticsRepository(
        _dummyClient(),
        rpcCaller: (fn, {params}) async => {
          'success': false,
    ...
    test(
      'rejects a start date after the end date without calling the RPC',
      () async {
        var called = false;
        final repo = SdgAnalyticsRepository(
          _dummyClient(),
