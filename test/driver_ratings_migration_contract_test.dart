...
void main() {
  ...
  test('one rating per ride is enforced at the schema level', () {
    ...
    expect(sql, contains('score INTEGER NOT NULL CHECK (score BETWEEN 1 AND 5)'));
    expect(sql, contains('char_length(comment) <= 300'));
  });

  test('only the completed ride\'s own rider can insert a rating for the assigned driver', () {
    final policyStart = sql.indexOf('CREATE POLICY driver_ratings_rider_insert');
  ...
  test('drivers can only ever see aggregated feedback, never rider identity', () {
    expect(sql, isNot(contains('driver_ratings FOR SELECT USING (driver_id')));
    ...
    final viewStart = sql.indexOf('CREATE OR REPLACE VIEW driver_rating_summary');
    final viewEnd = sql.indexOf('GRANT SELECT ON driver_rating_summary', viewStart);
  ...
  test('ratings are never client-writable except through the guarded insert policy', () {
