...
// write and what Supabase's embedded-resource select in
// ReceiptRepository.loadReceipt returns, so a column rename or type change
// on either side of that boundary breaks this test instead of the app.
...
void main() {
  test('booking-to-receipt: a freshly booked ride parses route, category and fare breakdown', () {
  ...
  test('booking-to-refund: a cancelled+refunded ride parses fee, refund and final amount', () {
    ...
    // The amount actually retained plus the amount refunded must reconcile
    // back to what was quoted, or the receipt would show money vanishing.
    expect(payment.refundedAmount + payment.cancellationFee, payment.quotedAmount);
  });
  ...
  test('a ride with no fare_quotes/payments yet does not crash the receipt', () {
    final json = Map<String, dynamic>.from(_bookedRideJson());
