...
void main() {
  ...
  group('earnCompletionReward', () {
    test(
      'sends only rideId/paymentId — never a client-supplied user or points',
      () async {
        ...
        await repo.earnCompletionReward(
          rideId: 'ride-1',
          paymentId: 'payment-1',
        );
    ...
    test(
      'refuses an empty rideId or paymentId without calling the RPC',
      () async {
        var called = false;
        final repo = RewardsRepository(
          _dummyClient(),
    ...
    test(
      'throws RewardsException when the ride is not a completed+paid ride',
      () async {
        final repo = RewardsRepository(
          _dummyClient(),
          rpcCaller: (fn, {params}) async => {
